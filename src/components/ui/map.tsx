...
function useMapStyles(styles: MapProps["styles"], blank: boolean) {
...
function useMapViewport(
    map: MapLibreGL.Map | null,
    viewport: Partial<MapViewport> | undefined,
    ...
    }, [map, isControlled, viewport])

    return { internalUpdateRef, isControlled }
}

function useMapStyleSwitch(
    map: MapLibreGL.Map | null,
    resolvedTheme: Theme,
    ...
    }, [map, resolvedTheme, mapStyles, setStyleLoaded])

    useEffect(() => {
        if (!map || !pendingStyle) return
        setPendingStyle(null)
        styleSwapInFlightRef.current = true
        map.setStyle(pendingStyle, { diff: false })
    }, [map, pendingStyle])

    return { styleSwapInFlightRef, currentStyleRef }
}
...
const Map = forwardRef<MapRef, MapProps>(function Map(
    ...
    const mapStyles = useMapStyles(styles, blank)
    const { internalUpdateRef } = useMapViewport(mapInstance, viewport, onViewportChange)
    const { styleSwapInFlightRef, currentStyleRef } = useMapStyleSwitch(
        mapInstance,
        resolvedTheme,
        mapStyles,
        setIsStyleLoaded,
    ...
    }, [mapInstance, isStyleLoaded, projection, styleSwapInFlightRef])
...
type MapMarkerDragHandlers = {
    start?: (lngLat: { lng: number; lat: number }) => void
    move?: (lngLat: { lng: number; lat: number }) => void
    end?: (lngLat: { lng: number; lat: number }) => void
}
...
type MapMarkerProps = {
    ...
    /** Drag event handlers (requires draggable: true) */
    onDrag?: MapMarkerDragHandlers
...
function MapMarker({
    ...
    useEffect(() => {
        ...
        if (current.lng !== position.lng || current.lat !== position.lat) {
    ...
    }, [marker, position.lng, position.lat, draggable, offset, rotation, rotationAlignment, pitchAlignment])
...
function PopupCloseButton({ onClick }: { onClick: () => void }) {
    return (
        <button
            ...
            className="hit-area-3 absolute top-1 right-1 z-10 inline-flex size-5 cursor-pointer items-center justify-center rounded-sm text-foreground transition-colors hover:bg-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset"
        >
...
type MapControlsProps = {
    ...
    /** Control visibility: { zoom, compass, locate, fullscreen } */
    controls?: {
        zoom?: boolean
        compass?: boolean
        locate?: boolean
...
function ControlButton({
    ...
    return (
        <button
            ...
            type="button"
            className={cn(
                "hit-area-2 flex size-8 items-center justify-center transition-colors",
...
function MapControls({ position = "bottom-right", controls, className, onLocate }: MapControlsProps) {
...
function MapArc<T extends MapArcDatum = MapArcDatum>({
    ...
    useEffect(() => {
        ...
        const handleMouseMove = (e: MapLibreGL.MapLayerMouseEvent) => {
            ...
            if (featureId === undefined || featureId === null || featureId === hoveredId) return
